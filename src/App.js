...
function App() {
  ...
  //Criamos uma mensagem que vai ficar mostrando uma mensagem até que o conteudo da página tenha sido gerado completamente.
  ...
  async function carregarTodosPersonagens() {
    ...
    const result = await fetch(
      ...
      'https://rickandmortyapi.com/api/character/?species=robot&gender=male&status=unknown',
      requestOptions
    )
  ...
  async function montarPersonagens() {
    const personagens = await carregarTodosPersonagens();

    return personagens.map(personagem => //.map vai mapear todos os itens de dentro do array
  ...
  //Dispara uma funcção, que dispara alguma coisa, quando algo acontecer
  useEffect(() => {
    async function pegarConteudo() {
      setConteudo(await montarPersonagens())
    }
